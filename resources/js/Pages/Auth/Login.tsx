...
export default function Login({
    ...
    return (
        <GuestLayout>
            ...
            <h2 className="text-xl font-bold text-foreground text-center mb-1">Connexion</h2>
            <p className="text-sm text-muted-foreground text-center mb-6">Accédez à votre espace administrateur</p>
            ...
            <a
                ...
                className="w-full flex items-center justify-center gap-3 bg-surface border border-border rounded-xl px-4 py-3 text-sm font-medium text-foreground hover:bg-surface-hover transition-colors mb-6"
            >
            ...
            <form onSubmit={submit} className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                    ...
                    {canResetPassword && (
                        <Link href={route('password.request')} className="text-sm text-primary hover:text-primary/80">
                            Mot de passe oublié ?
                        </Link>
                    )}
                </div>
                ...
                >
                    {processing ? <Loader2 className="w-5 h-5 animate-spin" /> : 'Se connecter'}
                </button>

                <p className="text-center text-sm text-muted-foreground">
                    Pas encore de compte ?{' '}
                    <Link href={route('register')} className="text-primary hover:text-primary/80 font-medium">
                        S'inscrire
                    </Link>
                </p>
            </form>
